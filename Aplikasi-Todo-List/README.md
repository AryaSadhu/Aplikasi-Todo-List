...
## Jobdesk: Security & Testing
**Nama:** I Gusti Ngurah Ananda Mahesya Suputra
**NIM:** 240030046
**Role:** Security & Testing

## Deskripsi bagian yang dibuat:
A. SECURITY (KEAMANAN SISTEM)

Security bertujuan untuk melindungi data pengguna, mencegah akses tidak sah, dan menjamin integritas sistem. Pada aplikasi To Do List ini, aspek keamanan diterapkan melalui beberapa file inti berikut.

1. .htaccess

Deskripsi :

File .htaccess digunakan untuk mengatur konfigurasi keamanan pada server Apache tanpa perlu mengubah file konfigurasi utama server.


Penjelasan Security :

- Mencegah directory listing, sehingga pengguna tidak dapat melihat isi folder secara langsung.

- Membatasi akses ke file sensitif seperti konfigurasi dan API.

- Dapat digunakan untuk pengamanan URL, redirect, dan proteksi file.


Tujuan Keamanan :

- Melindungi struktur folder aplikasi

- Mencegah eksploitasi melalui akses langsung ke server
...
File ini berisi konfigurasi koneksi database yang digunakan oleh seluruh aplikasi.


Penjelasan Security :

- Menyimpan informasi penting seperti host, username, password database.

- Menggunakan koneksi terpusat (PDO/MySQLi) untuk mencegah SQL Injection.

- Memisahkan konfigurasi dari logika aplikasi.



Tujuan Keamanan :

- Menjaga kerahasiaan kredensial database

- Mengurangi risiko manipulasi query
...
File ini bertanggung jawab untuk mengelola session pengguna.
...
- Menyimpan identitas user yang login

- Mengontrol session timeout
...
- Menjaga status login pengguna tetap valid
...
File ini digunakan untuk memastikan bahwa hanya pengguna yang sudah login yang dapat mengakses halaman tertentu.


Penjelasan Security :

- Mengecek apakah session login tersedia

- Mengarahkan user ke halaman login jika belum terautentikasi
...
- Melindungi halaman penting seperti dashboard dan manajemen task



5. api/login_process.php

Deskripsi :

File ini berguna untuk memproses data login yang dikirim oleh user.
...
- Pencocokan password dengan hash di database

- Menyimpan session login jika berhasil


Tujuan Keamanan :

- Mencegah login yang tidak sah

- Mengamankan data kredensial pengguna



6. api/register_process.php

Deskripsi :

File ini berfungsi untuk menangani proses registrasi pengguna baru.
...
- Mencegah manipulasi input dan akun palsu
...
File ini berisi kumpulan fungsi umum yang digunakan di berbagai bagian aplikasi.
...
- Mengurangi risiko XSS (Cross Site Scripting)
...
- Mengamankan aplikasi dari input berbahaya
...
Testing dilakukan untuk memastikan bahwa sistem berjalan sesuai kebutuhan, bebas error, dan aman digunakan. Pada aplikasi ini, testing dilakukan secara manual (black box testing).
...
- Memastikan sistem autentikasi berjalan dengan benar
...
- Memastikan validasi data berjalan optimal
...
- Memastikan fitur create task berfungsi


4. pages/edit_task.php
...
- Menguji perubahan data


Tujuan Testing :

- Memastikan user hanya bisa mengubah task miliknya


5. pages/delete_task.php
...
- Memastikan keamanan proses delete data


6. api/task_process.php
...
- Untuk menjamin endpoint API aman dan stabil


7. api/search_task.php

Pengujian :

- Pencarian task normal

- Input kosong

- Karakter khusus (uji SQL Injection)


Tujuan Testing :

- Untuk keamanan fitur pencarian
